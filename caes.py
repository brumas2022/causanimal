...
def bdanimais():
   ...
   total_macho = supabase.table("caninos").select("*").eq("genero","macho").eq("vivo", True).eq("adotado", False).execute()
...
def consulta_supa():
   ...
   key = st.secrets['SUPABASE_KEY']
   supabase: Client = create_client(url,key)
   selecao_genero = st.selectbox("Selecione o genero", ("Femea", "Macho"))
   total_macho = supabase.table("caninos").select("nome", "entrada", "historico", "foto").eq("genero", selecao_genero).eq("vivo", True).eq("adotado", False).execute()
   ...
   for post in total_macho.data:
               ...
               col[2].write("___")
               if post['foto']:
                  col[3].image(post['foto'], width=100)
                  col[3].write("___")
               else:
                  col[3].markdown("SEM FOTO", text_alignment="center")
                  col[3].write("___")
